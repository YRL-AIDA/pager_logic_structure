...
distance.
It is used to score participants in FinTOC2022 shared task.
...
USAGE
------
python metric.fintoc4.py--gt_folder <gt_folder> --submission_folder <submission_folder>
...
class Writer:
    ...
    def dump_toc(self):
        with open("toc_report.csv", "a", encoding='utf-8') as outfile:
            writer = csv.writer(outfile, dialect=csv.excel, delimiter='\t')
            writer.writerows(self.toc_rows)

    def dump_td(self):
        with open("td_report.csv", "a", encoding='utf-8') as outfile:
            writer = csv.writer(outfile, dialect=csv.excel, delimiter='\t')
...
def score_title_detection(toc1, toc2, log):
    ...
    for entry1 in toc1.entries:
        res = find_matching_entry(entry1.text, entry1.page_nb, toc2)
        if res is not None:
...
def find_matching_entry(text, page, toc):
    if len(toc.entries) == 0:
        return None
    similarities = []
    for entry in toc.entries:
        if not entry.matched:
            similarities.append(Levenshtein.ratio(text.lower().replace(" ", ""), entry.text.lower().replace(" ", "")))
        else:
        ...
        while True:
            ...
            try:
                if link_result == 0:
                    ...
                    text_similarity = Levenshtein.ratio(entry1.text.lower().replace(" ", ""),
                                                        entry2.text.lower().replace(" ", ""))
...
def score(folder1, folder2):
    ...
    for json1 in ls(folder1, JSON_EXTENSION):
        ...
        if not os.path.isfile(json2):
            ...
            writer.td_rows.append([doc_id, td_prec[docname], td_reca[docname],
                                   td_f1[docname], docname])
            # TOC generation
    ...
    writer.toc_rows.append(get_avg_row(team, xerox, inex, count))
    writer.toc_rows.append(get_std_row(xerox, inex))
    writer.td_rows.append(get_avg_row(team, td_prec, td_reca, td_f1))
    writer.td_rows.append(get_std_row(td_prec, td_reca, td_f1))
    # get stats about missing and added docs
    writer.toc_rows.append([f"Done: {len(docnames)} comparisons for {len(docnames1)} in groundtruth and {len(docnames2)} in submission"])
    if n_missing_docs:
...
@overload(str, XeroxMetric, InexMetric, Stats)
def get_avg_row(team, xerox, inex, count):
    out = []
    out.append("%4s " % team)
    # xerox
...
@overload(str, dict, dict, dict)
def get_avg_row(team, td_prec, td_reca, td_f1):
    return [
        team,
...
if __name__ == '__main__':
    ...
    JSON_EXTENSION = ".pdf.fintoc4.json"
    VERBOSE = True
    STRING_THRESHOLD = 0.85
    PARSER = argparse.ArgumentParser(description='This is the scoring script\
                                     used for FincTOC2022. It outputs two csv\
                                     reports, one for title detection, and\
                                     another for toc generation. It also logs\
                                     information in two separate log files.')
    PARSER.add_argument('--gt_folder', required=True, type=str, help="path to"
                        " folder containing groundtruth files (one groundtruth"
                        "file in json format per document")
    PARSER.add_argument('--submission_folder', required=True, type=str,
                        help="path to folder containing submission files (one"
                        " submission file in json format per document")
